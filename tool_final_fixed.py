...
def dns_lookup(target, mode="cepat"):
    ...
    try:
        ...
        _joined_result_0 = '\n'.join(results)
        _cleaned_result_0 = clean_ansi_codes(_joined_result_0)
        telegram_msg = f"<b>🌐 DNS LOOKUP FOR {target}</b>\n<pre>{_cleaned_result_0}</pre>"
        send_to_telegram(telegram_msg)
    except Exception as e:
        console.print(f"[red]Error: {str(e)}[/red]")
...
def http_header_analysis(target):
    ...
    try:
        ...
        telegram_msg = f"<b>📋 HTTP HEADERS FOR {target}</b>\n" + \
                       ...
                       _joined_result_1 = '\n'.join(results)
                       _cleaned_result_1 = clean_ansi_codes(_joined_result_1)
                       f"{security_status}\n<pre>{_cleaned_result_1}</pre>"
        send_to_telegram(telegram_msg)
    except Exception as e:
        console.print(f"[red]Error: {str(e)}[/red]")
...
def injection_tester(target):
    ...
    if "?" in target:
        ...
        form_params = console.input("[bold green]Enter form parameters (e.g. param1=value1&param2=value2): [/]").strip()        is_form = True
...
def install_dependencies():
    ...
    _joined_result_4 = '\n'.join(results)
    _cleaned_result_4 = clean_ansi_codes(_joined_result_4)
    telegram_msg = f"<b>⚙️ DEPENDENCY INSTALLATION</b>\n<pre>{_cleaned_result_4}</pre>"
    send_to_telegram(telegram_msg)
...
def main_menu():
    ...
    while True:
        ...
        console.print("30. Update Toolkit - Perbarui toolkit dan dependensi")
        console.print("31. Install Dependencies - Instal semua dependensi")
        console.print("32. About - Informasi tentang toolkit")
        console.print(" 0. Exit - Keluar dari aplikasi")
        console.print("="*60)

        choice = console.input("[bold green]Select option (0-32): [/]").strip()
