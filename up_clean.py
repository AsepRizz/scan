...
# Ganti di environment: TELEGRAM_TOKEN dan TELEGRAM_CHAT_ID
...
def send_to_telegram(message):
    """Kirim hasil scan ke Telegram (opsional, jika token diset)"""
    try:
...
def banner():
    ...
    console.print(f"[bold red]Reconnaissance Toolkit with FAST/COMPLETE Modes[/bold red]\n")
...
def subdomain_checker(target, mode="cepat"):
    ...
    try:
        ...
        subdomains = [s for s in result_asset.stdout.splitlines() if s.strip()]
        ...
        if mode == "lengkap" and ensure_tool("httprobe", "go install github.com/tomnomnom/httprobe@latest"):
            ...
            with subprocess.Popen(["httprobe"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, text=True) as probe:
                probe.stdin.write("\n".join(subdomains))
                probe.stdin.close()
                live_subs = [s for s in probe.stdout.read().splitlines() if s.strip()]
...
def screenshot_web(target, mode="cepat"):
    ...
    try:
        ...
        telegram_msg = f"<b>🖼️ SCREENSHOT UNTUK {target}</b>\nScreenshot berhasil disimpan di folder: {outdir}"
        send_to_telegram(telegram_msg)
    except Exception as e:
        console.print(f"[red]Error: {str(e)}[/red]")
...
def menu():
    ...
    while True:
        table = Table(title="RECON TOOLKIT", header_style="bold red", show_lines=True)
        ...
        mode = "cepat"
        # Minta pilihan mode hanya untuk fitur yang relevan
        if choice in ["3", "5"]:
...
if __name__ == "__main__":
    try:
        ...
        send_to_telegram("🟢 <b>ASEPSCAN DIMULAI</b>\\nTools reconnaissance siap digunakan!")
